process HIFIASM {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/hifiasm:0.19.5--h5b5514e_0' :
        'quay.io/biocontainers/hifiasm:0.19.5--h5b5514e_0' }"

    input:
    tuple val(meta), path(reads)
    tuple val(meta), path(paternal_kmer_dump)
    tuple val(meta), path(maternal_kmer_dump)
    path  hic_read1
    path  hic_read2
