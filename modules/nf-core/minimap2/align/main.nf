process MINIMAP2_ALIGN {
    ...
    def prefix = task.ext.prefix ?: "${meta.id}.${reads.getSimpleName()}" // samtools cat will need unique filenames - not true but still sane
    def bam_output = bam_format ? "-a | samtools sort -@ ${task.cpus/2} | samtools view -@ ${task.cpus/2} -b -h -o ${prefix}.bam" : "-o ${prefix}.paf"
