...
workflow NALLO {
    ...
    if (!val_skip_snv_calling) {
        ...
        // Per-family set of family IDs with a non-empty mito BED; used downstream to gate +1 on num_intervals.
        // call_mitochondrial_variants already filters bed.size() > 0 internally — this mirrors that check so
        // groupKey at CONCAT_SORT_RANKED_SNVS does not expect a mito VCF that will never arrive.
        ch_mito_active_families = SCATTER_GENOME.out.bed_mitochondrial_intervals
            .filter { _meta, bed, _num_intervals -> bed.size() > 0 }
            .map { meta, _bed, _num_intervals -> meta.id }
            .collect()
            .map { fam_ids -> fam_ids as Set }
    ...
    if (!val_skip_phasing) {
        ...
        ch_snv_vcf_tbi_nuclear_for_annotation = BCFTOOLS_VIEW_PHASING.out.vcf
            ...
            .combine(ch_mito_active_families)
            .map { meta, vcf, tbi, mito_families ->
                def add_mito = !val_skip_mitochondrial_calling && mito_families.contains(meta.family_id)
