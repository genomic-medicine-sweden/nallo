...
workflow NALLO {
    ...
    if (!val_skip_alignment) {
        ...
        if (!val_premapped && !val_precalled) {
    ...
    if (!val_skip_genome_assembly) {
        ...
        CONVERT_INPUT_BAMS(
            val_skip_alignment || val_premapped || val_precalled || (val_alignment_processes == 1) ? ch_samplesheet : SPLITUBAM.out.bam.transpose(),
            true,
            false,
        )
    ...
    if (!val_skip_alignment) {
        ...
        if (!val_premapped) {
            ...
            // If bams are premapped or precalled, just merge them (ONT machines output several BAMs per sample)
    ...
    if (!val_skip_snv_calling) {
        ...
        if (!val_precalled) {
            ...
            // Read pre-called family SNV VCF from samplesheet meta; one VCF per family
            ch_precalled_family_snv = ch_samplesheet
                .map { meta, _file -> [[id: meta.family_id], file(meta.snv_vcf), file(meta.snv_vcf_tbi)] }
    ...
    // Call SVs
    //
    // Initialise SV merge output channels so they are always defined
    ch_merge_svs_family_vcf = channel.empty()
    ch_merge_svs_family_tbi = channel.empty()
    ch_svs_per_family_and_caller_vcf = channel.empty()
    ch_svs_per_family_and_caller_tbi = channel.empty()
    ...
    if (!val_skip_sv_calling) {
        ...
        if (!val_precalled) {
            ...
            ch_svs_per_family_and_caller_vcf = MERGE_SVS.out.family_caller_vcf
            ch_svs_per_family_and_caller_tbi = MERGE_SVS.out.family_caller_tbi
        }
        else {
            // Read pre-called family SV VCF from samplesheet meta; one VCF per family
            ch_precalled_family_sv = ch_samplesheet
                .map { meta, _file -> [[id: meta.family_id], file(meta.sv_vcf), file(meta.sv_vcf_tbi)] }
                .unique { meta, _vcf, _tbi -> meta.id }
    ...
    if (!val_skip_phasing) {
        ...
        if (!val_precalled) {
            ...
            // Precalled: the supplied SNV VCF is already a whole-genome family VCF; use it directly for phasing
        ...
        PHASING(
            ch_phasing_snv_vcf,
            ch_phasing_snv_tbi,
            ch_merge_svs_family_vcf,
            ch_merge_svs_family_tbi,
            ch_bam_bai,
    ...
    if (split_family_vcf_for_chromograph || (!val_skip_peddy && !val_skip_snv_annotation)) {
        ...
        ch_concat_sort_annotated_snvs_input = ch_unfiltered_vcf
            .join(ch_unfiltered_tbi, failOnMismatch: true, failOnDuplicate: true)
            .map { meta, vcf, tbi ->
    ...
    qc_bcftools_stats                   = (val_skip_snv_calling || val_precalled) ? channel.empty() : QC_SNVS.out.stats // channel: [ val(meta), path(txt) ]
    qc_deepvariant_vcfstatsreport       = (val_skip_snv_calling || val_precalled) ? channel.empty() : QC_SNVS.out.vcfstatsreport // channel: [ val(meta), path(html) ]
    ...
    snvs_family_joint_tbi               = (val_skip_snv_calling || val_skip_phasing || val_precalled) ? channel.empty() : BCFTOOLS_CONCAT_PHASING.out.tbi // channel: [ val(meta), path(tbi) ] — whole-genome pre-phasing family SNV VCF index; use as snv_vcf_tbi input with --precalled
    snvs_family_joint_vcf               = (val_skip_snv_calling || val_skip_phasing || val_precalled) ? channel.empty() : BCFTOOLS_CONCAT_PHASING.out.vcf // channel: [ val(meta), path(vcf) ] — whole-genome pre-phasing family SNV VCF; use as snv_vcf input with --precalled
    ...
    svs_per_family_and_caller_tbi       = ch_svs_per_family_and_caller_tbi // channel: [ val(meta), path(tbi) ]
    svs_per_family_and_caller_vcf       = ch_svs_per_family_and_caller_vcf // channel: [ val(meta), path(vcf) ]
    svs_per_family_merged_tbi           = (val_skip_sv_calling || val_precalled) ? channel.empty() : ch_merge_svs_family_tbi // channel: [ val(meta), path(tbi) ] — pre-phasing merged family SV VCF index; use as sv_vcf_tbi input with --precalled
    svs_per_family_merged_vcf           = (val_skip_sv_calling || val_precalled) ? channel.empty() : ch_merge_svs_family_vcf // channel: [ val(meta), path(vcf) ] — pre-phasing merged family SV VCF; use as sv_vcf input with --precalled
