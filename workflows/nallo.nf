...
workflow NALLO {
    ...
    repeat_trgt_sample_bam = (val_skip_repeat_calling || val_str_caller != "trgt" || cram_output) ? channel.empty() : CALL_REPEAT_EXPANSIONS_TRGT.out.sample_bam // channel: [ val(meta), path(bam) ]
    repeat_trgt_sample_bai = (val_skip_repeat_calling || val_str_caller != "trgt" || cram_output) ? channel.empty() : CALL_REPEAT_EXPANSIONS_TRGT.out.sample_bai // channel: [ val(meta), path(bai) ]
