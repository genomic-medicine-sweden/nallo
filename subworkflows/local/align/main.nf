...
workflow ALIGN {
    ...
    take:
    ch_bam              // channel: [mandatory] [ val(meta), path(reads), path(index) ]
    ...
    ch_bam
        .branch { _meta, _bam, bai ->
            unmapped: !bai
            mapped: bai
        }
        .set { ch_bam_separated }

    if (val_split_alignment) {
        ch_bam_separated.unmapped
             .map { meta, bam, _bai -> [meta, bam] }
             .set { ch_splitubam_in }
        SPLITUBAM(ch_splitubam_in)
        ch_unmapped = SPLITUBAM.out.bam
    } else {
        ch_unmapped = ch_bam_separated.unmapped
    }
