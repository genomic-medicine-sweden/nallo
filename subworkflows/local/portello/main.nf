...
workflow PORTELLO {
    ...
    RUN_PORTELLO(
        ch_assembly_to_reference_bam_bai.join(ch_reads_to_assembly_bam_bai, failOnMismatch: true, failOnDuplicate: true).combine(ch_fasta.map { _meta, fasta -> fasta }).map { meta, assembly_aligned_to_reference_bam, assembly_aligned_to_reference_bai, reads_aligned_to_assembly_bam, reads_aligned_to_assembly_bai, reference_fasta ->
            [
                meta,
