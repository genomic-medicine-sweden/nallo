...
!!!tip
    ...
    By default, samples are compared to background cohort with the same sex as found in the sample metadata. If the background cohort tsv file labels are not like the default MALE and FEMALE, this can be changed with --methbat_male_label and --methbat_female_label. Additional labels can be added with --profile-label through the --extra_methbat_options parameter.
