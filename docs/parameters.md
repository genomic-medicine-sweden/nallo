...
| `precalled` | Use pre-called VCFs from the samplesheet (snv_vcf, sv_vcf columns) to skip variant calling and re-run phasing and annotation only. The samplesheet file column must contain an aligned BAM; implies --premapped behaviour. | `boolean` | False |  |  |
